...
{
    /// <summary>
    ///     This class isolates a bunch of glue between MJ and the PDG optimizer that I don't
    ///     yet understand how to write correctly.
    /// </summary>
    public class MechJebModulePDGGlueBall : ComputerModule
    {
        ...
        public MechJebModulePDGGlueBall(MechJebCore core) : base(core) { }
        ...
        private void HandleDoneTask()
        {
            ...
            try
            {
                ...
                Optimizer? pdg = _ascent.GetOptimizer();
                if (pdg == null)
                    return;

                LastLmStatus     = pdg.TerminationType;
                LastLmIterations = pdg.Iterations;
                LastZnorm        = pdg.PrimalFeasibility;

                if (LastLmIterations > MaxLmIterations)
                    MaxLmIterations = LastLmIterations;

                if (pdg.Success() && pdg.Solution != null)
                {
                    Core.Guidance.SetSolution(pdg.Solution);
                ...
                {
                    Debug.Log("failed guidance, znorm: " + pdg.PrimalFeasibility);
                }
            }
            finally
            {
